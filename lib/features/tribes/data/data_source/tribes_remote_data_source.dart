...
abstract class TribesRemoteDataSource {
  Future<List<TribesData>> getTribes({required int perPage});
...
class TribesRemoteDataSourceImpl implements TribesRemoteDataSource {
  ...
  @override
  Future<List<TribesData>> getTribes({required int perPage}) async {
    final response = await apiService.get(endPoint: 'tribes?per_page=$perPage');
